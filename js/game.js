...
//游戏原理：
/*
	单体模式（单例模式）： 避免了全局的污染，把所有方法和属性都是放在一个对象里面的，与其它其它对象不会发生什么冲突的
						   好处：
	...
	整个游戏程序它是以单体模式或者叫单例模式开发的，
	所有的方法属性都是封装在一个对象下面的，所有的方法都是在我们提供的一个接口里面执行的
	...
	难度级别控制：
			...
			难度主要控制的是敌机的生成速度与敌机下落速度，子弹的生成速度与子弹的发射速度，限制了敌机漏掉的个数
...
var oGame = {
	...
	"init":function(){
		...
		for(var j = 0;j<btns.length;j++){
			...
			btnArr[j].onclick = function(event){
				...
				oGame.startGame(ev,imgName[this.index],this.enemyspeed,this.bulletspeed,this.downspeed,this.missnum,this.speed);
				//计算分数
				oGame.score();
				//当前miss次数
				oGame.amount = 0;
	...
	*/
	...
	//开始游戏方法
	"startGame":function(ev,imgName,enemyspeed,bulletspeed,downspeed,missnum,speed){
		...
		oGame.oPlane(ev,bulletspeed,speed);
	...
	//我方飞机
	oPlane:function(ev,bulletspeed,speed){
